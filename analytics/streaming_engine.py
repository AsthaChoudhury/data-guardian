...
class DataQualityStreamingEngine:
        ...
        try:
            ...
            if issue_count > 0:
                ...
                cache_data = {
                    ...
                    "quality_metrics": {
                        "avg_quality_score": float(batch_df.select(col("quality_score")).collect()[0][0])
                    }
                }
    ...
    def run(self):
        ...
        try:
            ...
            df_completeness = self.check_completeness(df_parsed)
            df_validity = self.check_validity(df_completeness)
            df_anomalies = self.detect_anomalies(df_validity)

            df_with_quality = self.calculate_quality_score(df_anomalies)
